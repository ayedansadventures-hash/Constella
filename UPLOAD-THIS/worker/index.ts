...
async function handleChat(request: Request, env: Env): Promise<Response> {
  try {
    ...
    const isUltra = sysMsg.includes("ULTRA MODE ACTIVE");
    ...
    if (model === "kimi") {
      ...
      if (!apiRes.ok) {
        ...
        return new Response(JSON.stringify({ error: `Kimi API failed: ${errText}` }), { status: apiRes.status, headers: { "Content-Type": "application/json" } });
      }
      const data: any = await apiRes.json();
      return new Response(JSON.stringify({ content: data.choices?.[0]?.message?.content || "Error: empty response" }), { headers: { "Content-Type": "application/json" } });
    }
    ...
    if (model === "deepseek") {
      ...
      if (!apiRes.ok) {
        const errText = await apiRes.text().catch(() => "Unknown error");
        return new Response(JSON.stringify({ error: `DeepSeek API failed: ${errText}` }), { status: apiRes.status, headers: { "Content-Type": "application/json" } });
      }
      const data: any = await apiRes.json();
      return new Response(JSON.stringify({ content: data.choices?.[0]?.message?.content || "Error: empty response" }), { headers: { "Content-Type": "application/json" } });
    }
    ...
    if (model === "claude") {
      ...
      const claudeModel = isUltra ? "claude-fable-5" : "claude-sonnet-5";
      
      const apiRes = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        signal: AbortSignal.timeout(15000),
        headers: { "Content-Type": "application/json", "x-api-key": env.ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01" },
        body: JSON.stringify({ model: claudeModel, max_tokens: 1024, system: sysMsg, messages: otherMsgs })
      });
      if (!apiRes.ok) {
        const errText = await apiRes.text().catch(() => "Unknown error");
        return new Response(JSON.stringify({ error: `Claude API failed: ${errText}` }), { status: apiRes.status, headers: { "Content-Type": "application/json" } });
      }
      const data: any = await apiRes.json();
      return new Response(JSON.stringify({ content: data.content?.[0]?.text || "Error: empty response" }), { headers: { "Content-Type": "application/json" } });
    }
    ...
    if (model === "gemini") {
      ...
      if (!apiRes.ok) {
        const errText = await apiRes.text().catch(() => "Unknown error");
        return new Response(JSON.stringify({ error: `Gemini API failed: ${errText}` }), { status: apiRes.status, headers: { "Content-Type": "application/json" } });
      }
      const data: any = await apiRes.json();
      return new Response(JSON.stringify({ content: data.candidates?.[0]?.content?.parts?.[0]?.text || "Error: empty response" }), { headers: { "Content-Type": "application/json" } });
    }
    
    return new Response(JSON.stringify({ error: "Unknown model" }), { status: 400 });
  } catch (err: any) {
    const isTimeout = err.name === "TimeoutError" || err.message?.includes("timeout");
    return new Response(JSON.stringify({ error: isTimeout ? "Request timed out after 15 seconds" : err.message }), { status: isTimeout ? 408 : 500, headers: { "Content-Type": "application/json" } });
  }
}
